...
my ($file);
foreach $file (qw(fink-depends.in install.sh COPYING VERSION
                  perlmod/Fink fink-depends.info.in
                 )) {
  if (not -e $file) {
...
$script .= "sed -e 's/\@VERSION\@/$packageversion/' -e 's/\@REVISION\@/$packagerevision/' <fink-depends.info.in >$basepath/fink/dists/local/main/finkinfo/utils/fink-depends-$packageversion.info\n";
...
if (&execute("$basepath/bin/fink install fink-depends")) {
  print "\n";
  &print_breaking("Installing the new fink-depends package failed. ".
  ...
  print "\n";
  &print_breaking("Your fink-depends installation in '$basepath' was updated with ".
		  "a new fink package.");
}
print "\n";
